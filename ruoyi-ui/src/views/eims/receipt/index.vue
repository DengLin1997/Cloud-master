<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="orderList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="待付金额" align="center" prop="waitMoney" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="1000px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="订单总额" prop="totalAmount">
              <el-input v-model="form.totalAmount" placeholder="" disabled="disabled" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="扣除定金" prop="deposit" >
              <el-input v-model="form.deposit" placeholder=""  disabled="disabled" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="本次付款" prop="paymentMoney">
              <template slot-scope="scope">
                <el-input v-model="form.paymentMoney" placeholder="" @input="payment(scope.row)" />
              </template>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="本次欠款" prop="arrears">
              <el-input v-model="form.arrears" placeholder="" disabled="disabled"/>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
import { listReceipt, getReceipt, delReceipt, addReceipt, updateReceipt } from "@/api/eims/receipt";
...
export default {
  ...
  methods: {
    ...
    cancel1() {
      ...
      this.form.deposit =this.purchaseOrderListInfo.map(row => row.deposit).reduce((acc, cur) => (cur + acc), 0)
      this.form.totalAmount =this.purchaseOrderListInfo.map(row => row.totalAmount).reduce((acc, cur) => (cur + acc), 0)-this.form.deposit
    },
    // 表单重置
    reset() {
      this.form = {
    ...
    getPurchaseOrderListInfo() {
      listOrder().then(response => {
        ...
        this.purchaseOrderList = this.purchaseOrderList.filter(v => v.state ===1)
      });
    },
    /** 采购单勾选事件 */
    getChange(row, key){
